        ...
        public static void AjouterUtilisateur(Utilisateur utilisateur)
        {
            ...
            try
            {
                if (utilisateur is null)
                    throw new ArgumentNullException(nameof(utilisateur), "L'utilisateur ne peut être null");
                if (ObtenirUtilisateurs().ContainsKey(utilisateur.Id))
                    throw new InvalidOperationException("L'utilisateur existe déjà");
                cn.Open();
                //public Depenses(string nom, int cat, decimal cout, DateTime date, TypeFrequence frequence, bool obligatoire)
